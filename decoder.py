...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        output = tgt # Refers to first feature (F^{0}_pos)
        T,B,C = memory.shape # Refer to one of the middle features (F^{n}_pos)
        intermediate = []
        ...
        for n, layer in enumerate(self.layers):
            ...
            if self.return_intermediate: # For inference is False
        ...
        return output # output.shape -> (num_frames, batch, 64)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.normalize_before: # For inference is True
