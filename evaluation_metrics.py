...
class MetricsEvaluator:
    ...
    def relaxed_evaluation_Cholec80(self, predLabelID, gtLabelID, fps=1):
        """
    ...
    def relaxed_evaluation_M2CAI(self, predLabelID, gtLabelID, fps=1):
        """
        Evalúa el desempeño de un modelo de reconocimiento de fases quirúrgicas.
    ...
    def calculate_metrics(self, preds, gts, metrics, dataset=None):
    ...
    def evaluation_per_dataset(self, preds, gts, videos_names, dataset_name):
        ...
        if dataset_name == "AutoLaparo":
            ...
            for idx, video in enumerate(videos_names):
                ...
                if video_preds and video_gts:
                    video_results.append(self.calculate_metrics(video_preds, video_gts, ["accuracy", "precision", "recall", 'jaccard', 'f1_score'], dataset_name))

                    acc, pre, rec, jac = self.relaxed_evaluation_Cholec80(np.array(video_preds), np.array(video_gts))
            ...
            # Perform video-level evaluation
            dataset_info = {'Video_id': names, 'Pred': dataset_preds, 'GT': dataset_gts}
            df = pd.DataFrame(dataset_info)
            video_results = []
            ...
            for idx, video in enumerate(videos_names):
                ...
                if video_preds and video_gts:
                    ...
                    acc, pre, rec, jac = self.relaxed_evaluation_M2CAI(np.array(video_preds), np.array(video_gts))
